...
export class GiftsList implements OnInit {
  ...
  onCategorySelected(category: string) {
    this.giftsService
      .getFiltered(12, category)
      .subscribe((filteredGifts) => (this.gifts = filteredGifts));
  }

  onDetailsViewed(gift: IGift) {
  ...
  onAddedToCart(gift: IGift) {
    this.cartService.addItem(gift);
  }

  onQuantityIncreased(gift: IGift) {
  ...
  onQuantityDecreased(gift: IGift) {
    const item = this.items.find((item) => item.id === gift.id);
    if (item) {
